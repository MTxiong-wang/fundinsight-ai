...
# 工具评分模式：只输出工具评分（不运行AI）
python main.py --tool-only 芯片
python main.py -t 半导体  # 简写
```

**输出位置**: `outputs/{板块名}_{日期}.md`

**两种模式区别**：
- **默认模式**：直接调用AI评分，输出AI的分析结果（推荐）
- **工具评分模式**：使用工具的评分标准，输出相对评分结果
...
【4】完成
    ...
    - Prompt文件: outputs/{板块名}_{日期}_prompt.md（仅AI模式）
```
...
- ✅ 新增两种运行模式：默认AI评分 / 工具评分模式（--tool-only）
...
- [ ] **国证指数支持**
  - 当前项目只支持中证指数官网爬虫
  - 需要增加国证指数官网的数据爬取
  - 国证指数官网: https://www.cnindex.com.cn/
  - 实现方式: 复用现有Playwright架构，增加国证指数爬虫模块
